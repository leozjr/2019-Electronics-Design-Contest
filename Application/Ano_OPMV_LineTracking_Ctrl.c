...
#define LT_CMPPIXEL     0.005f     //每像素对应的地面距离，与焦距和高度有关，需要调试标定。//目前粗略标定
...
void ANO_LT_StepProcedure(u8 *dT_ms)
{
	...
	switch(step_pro_sta)
	{
		...
		{
			//刹车减速
			ano_opmv_lt_ctrl.exp_velocity_h_cmps[0] = 2;//识别到直角后减速前进的速度cm/s
...
//				if(opmv.lt.sta == 3)
		...
		{
			//
			ano_opmv_lt_ctrl.exp_velocity_h_cmps[0] = FORWARD_VEL;//边旋转边给前进速度，走出一定圆弧
...
void ANO_LTracking_Ctrl(u8 *dT_ms,u8 en)
{
	...
	if(en)
	{
		...
		{
			//
			ano_opmv_lt_ctrl.exp_yaw_pal_dps = -opmv.lt.angle *4;
